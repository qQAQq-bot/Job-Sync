...
- `paths.rs`, `settings.rs`, `storage/`, `sidecar/`, `worker.rs` — filesystem paths, settings persistence, local storage, sidecar control, and worker orchestration, including bundled Node runtime discovery for release/portable builds plus development-mode Node fallback.
